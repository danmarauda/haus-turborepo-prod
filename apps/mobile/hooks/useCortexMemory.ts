/**
 * HAUS + Cortex Memory Hook
 *
 * React Native hook that integrates Cortex memory with the HAUS voice agent:
...
export function useCortexMemory(
  ...
  const { convex, userId, enabled = true } = options;
  ...
  const recallForQuery = useCallback(
    async (query: string, limit: number = 20): Promise<RecallResult> => {
      if (!enabled) {
        return {
          memories: [],
          facts: [],
  ...
  const storePreference = useCallback(
    async (params: {
      ...
      try {
        ...
        const error = err as Error;
        setError(error);
        console.error('[useCortexMemory] Failed to store preference:', error);
        return { success: false, error: error.message };
      } finally {
        setIsLoading(false);
      }
...
export function useConversationMemory(
  ...
  const { convex, userId, enabled = true, onConversationStored } = options;
  const [isStoring, setIsStoring] = useState(false);
  const cortex = useCortexMemory({ convex, userId, enabled });
